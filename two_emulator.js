            ...
            message => {
                switch (message.command) {
                    ...
                    case 'loadAppetizeEmulator':
                        loadAppetizeEmulator(panel, message.deviceType, message.emulatorId);
                        break;
                }
            },
            undefined,
...
function getWebviewContent() {
    ...
    <head>
        ...
        <style>
            ...
            .emulator-frame { border: none; width: 100%; height: 900px; overflow: hidden; }
    ...
    <body>
        ...
        <div class="right-panel">
            <div class="panel">
                ...
                <div id="ios-emulator" class="emulator-container" style="display: flex;">
                    <div class="ios-emulators-wrapper">
                        <!-- First iOS Emulator -->
                        <div class="ios-emulator-container">
                            <div class="emulator-controls">
                                <select id="ios-device-1" class="device-select">
                                ...
                                <button onclick="loadAppetizeEmulator(1)">Load iOS Emulator 1</button>
                            </div>
                            <div id="appetize-container-1" class="appetize-container" style="height: 400px;">
                                <p>Loading iOS emulator 1...</p>
                            </div>
                        </div>
                        ...
                        <div class="ios-emulator-container">
                            <div class="emulator-controls">
                                <select id="ios-device-2" class="device-select">
                                    <option value="iphone15pro">iPhone 15 Pro</option>
                                    <option value="iphone14">iPhone 14</option>
                                    <option value="iphone13mini">iPhone 13 Mini</option>
                                    <option value="ipadpro">iPad Pro</option>
                                </select>
                                <button onclick="loadAppetizeEmulator(2)">Load iOS Emulator 2</button>
                            </div>
                            <div id="appetize-container-2" class="appetize-container" style="height: 400px;">
                                <p>Loading iOS emulator 2...</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            function loadAppetizeEmulator(emulatorId) {
                console.log(\`Requesting iOS emulator \${emulatorId} from extension\`);
                const deviceType = document.getElementById(\`ios-device-\${emulatorId}\`).value;
                vscode.postMessage({ 
            ...
            window.addEventListener('message', event => {
                ...
                if (message.type === 'appetize-emulator') {
                    console.log('Received Appetize URL:', message.url);
                    const emulatorId = message.emulatorId || 1;
                    const container = document.getElementById(\`appetize-container-\${emulatorId}\`);
...
function checkDevicesBeforeStartingScrcpy(panel) {
    ...
    exec('adb devices', (error, stdout, stderr) => {
        if (error) {
            ...
            // Load iOS emulators by default since no Android devices are available
            loadAppetizeEmulator(panel, 'iphone15pro', 1);
            loadAppetizeEmulator(panel, 'iphone14', 2);
            return;
        }

        const devices = stdout.trim().split('\n').slice(1).filter(line => line.trim() !== '');
        if (devices.length === 0) {
            console.log('No Android devices connected, loading iOS emulators by default');
            panel.webview.postMessage({ type: 'log', content: `\n${new Date().toISOString()}: No Android devices connected, loading iOS emulators by default` });
            // Load iOS emulators by default since no Android devices are available
            loadAppetizeEmulator(panel, 'iphone15pro', 1);
            loadAppetizeEmulator(panel, 'iphone14', 2);
...
function startEmulatorView(panel) {
    ...
    scrcpyProcess.stderr.on('data', (data) => {
        ...
        if (data.toString().includes('Could not find any ADB device') || 
            ...
            panel.webview.postMessage({ type: 'log', content: `\n${new Date().toISOString()}: Scrcpy failed to connect to Android device, switching to iOS emulators` });
            loadAppetizeEmulator(panel, 'iphone15pro', 1);
            loadAppetizeEmulator(panel, 'iphone14', 2);
...
function loadAppetizeEmulator(panel, deviceType = 'iphone15pro', emulatorId = 1) {
